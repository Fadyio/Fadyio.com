...
date: "2022-10-11"
tags: ["AWS", "EKS"]
description: "Set up an Amazon EKS cluster and deploy a sample Kubernetes application on AWS."
image: "./images/EKS-container.webp"
---

Kubernetes has become an industry standard for container orchestration.

Setting up and maintaining Kubernetes clusters in the cloud can still be challenging. You need to manage networking, control-plane access, security, updates, and patches.

Amazon Elastic Kubernetes Service (EKS) manages the Kubernetes control plane so you can focus on deploying and operating workloads.

This guide walks through setting up an EKS cluster and deploying a Kubernetes application on AWS.

## Step 0: Before you start
> 0. AWS account.
> 1. [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) installed on your computer. You can use the AWS Console to create a cluster, but this guide uses the CLI.
...
This article reflects the Amazon EKS workflow available when it was published in 2022. Kubernetes versions, CloudFormation templates, and recommended EKS access methods change over time, so compare each step with the current [Amazon EKS documentation](https://docs.aws.amazon.com/eks/latest/userguide/what-is-eks.html) before using it in production.
:::
...
![IAM Roles page with the Create role button highlighted](./images/screen1.webp)

From the list of AWS services, select EKS and then Next: Permissions at the bottom of the page.

![IAM trusted entity form with AWS service and EKS selected](./images/screen2.webp)

Select policies as in the image, and proceed to the Review page.

![EKS role permissions with cluster and service policies selected](./images/screen3.webp)

Enter a name for the role and hit the Create role button at the bottom of the page to create the IAM role.

![IAM role details naming the role EKSrole](./images/screen4.webp)

The IAM role is created.

![IAM confirmation that the EKSrole role was created](./images/screen5.webp)
...
![AWS CloudFormation landing page with Create stack highlighted](./images/screen6.webp)
...
![CloudFormation template page with the EKS VPC template URL](./images/screen7.webp)

Click Next and choose the name of the stack, leave the default network configurations as is, and click Next.

![CloudFormation EKS VPC stack name and network parameters](./images/screen8.webp)

Leave the default options untouched and then click Next and Next.

![CloudFormation review page for the EKS VPC stack](./images/screen9.webp)

Now, CloudFormation will begin to create the VPC. Once done, be sure to note the various values created Security Groups, Vpc ID and Sub-nets ID. You will need these in subsequent steps. You can see these under the Outputs tab of the CloudFormation stack.

![CloudFormation outputs listing the security group, subnet, and VPC IDs](./images/screen10.webp)
...
![EKS Clusters page showing the active MyEKSCluster cluster](./images/screen11.webp)
...
![Terminal showing kubectl get svc output for the Kubernetes service](./images/screen12.webp)

Click the cluster in the EKS Console to review configurations:

![EKS cluster overview showing status and configuration details](./images/screen13.webp)
...
```
![CloudFormation Create stack page with the EKS node group template URL](./images/screen14.webp)
...
![CloudFormation node group stack details for the cluster, scaling, instance type, and networking](./images/screen15.webp)

Click Next and check the box at the bottom of the page acknowledging that the stack might create IAM resources.
CloudFormation will create the worker nodes with the VPC settings we entered and three new EC2 instances will be created.

![EC2 instances list showing three running EKS worker nodes](./images/screen16.webp)

As before, once the stack is created, open Outputs tab:

![CloudFormation outputs showing the node instance role and node security group](./images/screen17.webp)
...
![Guestbook web application with a message field and Submit button](./images/screen18.webp)
